...
def average_performance(pred, target, thr=None, k=None):
    """Calculate class_precision, class_recall, class_f1, over_precision,
    ...
    class_precision = precision_class.mean() * 100.0
    class_recall = recall_class.mean() * 100.0
    class_f1 = 2 * class_precision * class_recall / np.maximum(class_precision + class_recall, eps)
    over_precision = tp.sum() / np.maximum(tp.sum() + fp.sum(), eps) * 100.0
    over_recall = tp.sum() / np.maximum(tp.sum() + fn.sum(), eps) * 100.0
    over_f1 = 2 * over_precision * over_recall / np.maximum(over_precision + over_recall, eps)
    return class_precision, class_recall, class_f1, over_precision, over_recall, over_f1
